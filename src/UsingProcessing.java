...
//		}
    ...
    public void draw(){
    		...
    		//fill(255);
    		rect(0, img.height, width, height-img.height);
    		for(int i = 0; i < pal.length; i++) {
    			fill(pal[i][0], pal[i][1], pal[i][2]);
    			System.out.println((width/pal.length) * i );
    			if(i == pal.length-1) {
    				rect( (width/pal.length) * i, img.height , (width/pal.length)+(width-(width/pal.length)), height-img.height);
    			}
    			else {
    				rect( (width/pal.length) * i, img.height , (width/pal.length), height-img.height);
    			}
    		}
